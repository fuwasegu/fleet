...
- **Agents that actually collaborate (A2A)** — connect cards with a curve to put their agents in one context channel. Via a bundled local MCP server each agent can:
  ...
  One thing to know: **a new card's agent starts when you open that card's terminal**, not the moment the card appears. So delegation gets the card set up for you — the right directory, CLI and model, already sharing context — but you still open it to set it going.
