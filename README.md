...
- **Agents that actually collaborate (A2A)** — connect cards with a curve to put their agents in one context channel. Via a bundled local MCP server each agent can:
  ...
  You drive this by asking in plain language, not by configuring anything: *"make a card in `~/repos/legacy` and have it work out how auth is done there"*, or *"spin up a Codex card to review this PR"*. The agent creates the real card — with its own directory, its own CLI and model — and the two of you end up in a channel, so you can then push work to it with `fleet_message` and read what it records with `fleet_recall`. This works from a card that isn't connected to anything yet: creating the card is what forms the channel, so the first delegation needs no setup.

  A delegated card **starts its agent by itself**, without you opening its terminal — so work really does proceed on a card you never touched. One caveat worth knowing: if the card's directory is one your CLI hasn't seen before, `claude` opens with its "do you trust this folder?" check and waits. Fleet shows that card as **Blocked** with the question on it, so the board tells you it needs a click rather than looking idle.
...
- **It taps you on the shoulder** — the moment an agent finishes (DONE) or stops for approval (BLOCKED), Fleet bounces its Dock icon, plays a sound, and puts the number of cards needing you on the Dock badge — so Fleet doesn't have to stay in front of you. Come back and "Needs attention" at the top names the cards; click one to land in its terminal. Nothing fires for a card whose terminal you already have open, and each kind can be toggled off in Settings.
...
- **Claude profiles** — define named profiles (a label like "Work" / "Personal" plus a `CLAUDE_CONFIG_DIR` path) in a small sheet, then assign one to a card. Fleet launches `claude` for that card with that `CLAUDE_CONFIG_DIR`, so different cards can run different Claude accounts/licenses; the card's session history and resume follow the same config dir. Fleet only sets the env var — the account still needs to be logged in under that config dir already.
- **A model per card** — type a model on a card and Fleet launches that card's agent with it (`--model` for Claude, a `model=` config override for Codex). Cheap-and-wide on the card that reads a lot, your strongest model on the one making the hard calls, and a different CLI on the one reviewing. It's a free-text field, not a fixed list, so a model released tomorrow works without updating Fleet — an alias like `opus` or a full id like `claude-opus-5` both do. Leave it blank for the CLI's default; existing cards are unaffected. Change it later from the card's right-click menu (it applies the next time that card's terminal starts).
