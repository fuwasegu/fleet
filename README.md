...
- **Agents that actually collaborate (A2A)** — connect cards with a curve to put their agents in one context channel. Via a bundled local MCP server each agent can:
  ...
  You drive this by asking in plain language, not by configuring anything: *"make a card in `~/repos/legacy`, have it work out how auth is done there, and send me what it finds"*, or *"spin up a Codex card and have it review this PR"*. The agent creates the real card — with its own directory, its own CLI and model — and the answer comes back into your session. `fleet_create_card` works even from a card that isn't connected to anything yet: creating the card is what puts the two of you in a channel, so the first delegation needs no setup.
