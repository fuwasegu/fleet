...
- **本当に協調する Agent 群 (A2A)** — カードを曲線でつなぐと、その Agent 群が1つの文脈チャンネルに入る。同梱のローカル MCP サーバ経由で:
  ...
  設定するのではなく、**自然語で頼めば動く**のがこの機構の使い方。「`~/repos/legacy` にカードを作って、そっちで認証の仕組みを調べさせて」「Codex のカードを立てて、この PR をレビューさせて」で通る。Agent が実際のカードを立て（ディレクトリ・CLI・モデルも指定できる）、双方が同じチャンネルに入るので、以降は `fleet_message` で仕事を渡し、`fleet_recall` で記録を読める。**まだどのカードともつながっていないカードからでも使える** — カードを作ることがチャンネルを作ることなので、最初の委譲に下準備が要らない。

  委譲でできたカードは、**あなたがターミナルを開かなくても自分で Agent を起動する**。触っていないカードで実際に作業が進む。一点だけ: そのディレクトリを CLI が初めて見る場合、`claude` は「このフォルダを信頼しますか？」の確認で待つ。Fleet はそのカードを **Blocked** として問いつきで表示するので、放置されているのではなく「クリックが必要」だと盤面から分かる。
- **エージェントの状態を一目で** — 動作中 / 承認待ち / 完了 / 待機 を各ターミナルから自動検出（OSC タイトル + 構造照合、herdr 方式）。承認待ちカードにはエージェントの *実際の問い* を表示。
- **手番が来たら知らせる** — エージェントが完了した（DONE）／承認待ちで止まった（BLOCKED）瞬間に、**Dock アイコンが跳ねて音が鳴り、要対応の件数が Dock バッジに出る**。Fleet を前面に置いておく必要がない。戻ってくれば上部の「要対応」にカード名が並ぶので、クリックすればそのターミナルへ直行。すでに開いているカードでは鳴らない。種別ごとに設定でオフにできる。
...
- **カードごとにモデルを指定** — カードにモデル名を書けば、そのカードのエージェントはそのモデルで起動する（Claude は `--model`、Codex は `model=` の設定上書き）。大量に読ませるカードは安く広く、難しい判断をさせるカードは一番強いモデル、レビューさせるカードは別 CLI、という使い分けができる。固定の選択肢ではなく自由入力なので、明日出たモデルも Fleet を更新せずに使える（`opus` のようなエイリアスでも `claude-opus-5` のような完全な ID でも通る）。空欄なら CLI の既定モデルで、既存カードは何も変わらない。後から変えたいときはカードの右クリックメニューから（次にそのカードのターミナルを起動したときから効く）。
...
- **Git worktree を Fleet が管理** — カード作成時に既存フォルダの代わりに worktree を新規作成できる。リポジトリ・ブランチ名・ベース(現在のブランチ or デフォルトブランチ)を指定すると、Fleet が `git worktree add` を実行してカードに紐付ける(既定では `../.fleet-worktrees/<branch>` 配下)。Fleet がその worktree を所有し、ターミナルも直接そこで起動するので、worktree 作業中でも git ブランチ/PR の検出が正しく効く。削除も安全設計: Fleet が作った worktree しか削除せず、未コミット/未 push の変更やセッション動作中は既定で削除を拒否してカードだけ削除する選択肢を出す。未コミットの変更が `.serena/` のような捨てて構わないゴミだけのときは、捨てられる差分(種類別の一覧 + ファイルごとの diff)を確認したうえで強制削除できる。その場合もブランチは残すのでコミットは失われない。agent 側からも `fleet_worktree_info` / `fleet_worktree_create` で自分のカードの worktree を参照・作成できる。
