...
- **本当に協調する Agent 群 (A2A)** — カードを曲線でつなぐと、その Agent 群が1つの文脈チャンネルに入る。同梱のローカル MCP サーバ経由で:
  ...
  設定するのではなく、**自然語で頼めば動く**のがこの機構の使い方。「`~/repos/legacy` にカードを作って、そっちで認証の仕組みを調べさせて、わかったことを送り返して」「Codex のカードを立てて、この PR をレビューさせて」で通る。Agent が実際のカードを立て、そのカードは自分のディレクトリと自分のセッションで作業し、答えがこちらに返ってくる。
