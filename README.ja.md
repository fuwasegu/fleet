...
- **エージェント間の文脈共有 (A2A)** — カードを曲線でつなぐと、その Agent 群が1つの文脈チャンネルに入る。各 Agent は同梱のローカル MCP サーバ経由で `fleet_recall` / `fleet_remember` / `fleet_peers` を使い、共有メモリを読み書きできる。並走する Agent の重複作業・食い違いを防ぐ。すべてローカル完結（クラウド不要）。
