...
## ターミナルの最低限の設定機能
- フォント（ファミリ/サイズ）、配色テーマ等を設定できるように。SwiftTerm はフォント/カラー設定APIあり。まずはフォントサイズから。

## Markdown プレビューの強化
- **mermaid** のレンダリング対応、**コードブロックのシンタックスハイライト**。
- これは現状のネイティブ行ベース描画では困難 → **WKWebView + marked.js + highlight.js + mermaid.js（アセット同梱）** に作り替える方針。
  - ターミナルと違い IME 不要の読み取り専用なので WKWebView 採用に問題なし。
